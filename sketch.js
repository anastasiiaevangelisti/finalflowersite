...
let musicPauseTime = 0; // Track the paused time of the music

// Animation variables
let showClickText = true;
let clickTextOpacity = 255; // Full opacity
...
function setup() {
  ...
  boundaryPositions = [
    ...
    createVector(width * 0.5, height * 0.9),
  ];
...
function draw() {
  ...
  // Animation for "click to hear sound"
  if (showClickText) {
    fill(245, 219, 255, clickTextOpacity);
    textSize(15);
    textFont(font);
    textAlign(CENTER, CENTER);
    text("click to hear sound", width / 2, height / 6);

    clickTextOpacity -= 5; // Reduce opacity
    if (clickTextOpacity <= 0) {
      showClickText = false; // Stop showing text
    }
  }
  ...
  if (distanceFromCenter < ellipseRadius) {
    ...
    if (!isMusicPlaying) {
      music.jump(musicPauseTime); // Resume from the saved time
    ...
    // Pause music and save the current time
    if (isMusicPlaying) {
      musicPauseTime = music.currentTime(); // Save the current playback time
...
class BoundaryText {
  constructor(x, y, displayText) {
    ...
    textFont(font);
    textSize(32);
    this.textBounds = font.textBounds(this.displayText, 0, 0, 32);
  }
